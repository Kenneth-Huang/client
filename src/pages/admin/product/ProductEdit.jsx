...
function ProductEdit({ history, match }) {
	...
	const loadSingleProduct = async (slug) => {
		await productRequest
			...
			.then(async (data) => {
				setProduct(data);
				await loadSubcategoriesByCategory(data.category._id);
			})
			.catch((err) => {
				toast.error('Error getting product:', err);
			});
	};
	...
	const handleSubmit = async (e) => {
		e.preventDefault();
		if (user && user.token && product && product.slug) {
			setLoading(true);
			await productRequest
				.updateProduct(user.token, product)
				.then((res) => {
					toast.success(res);
					//Redirect
